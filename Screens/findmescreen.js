...
export default function FindMeScreen({ navigation }) {
  ...
  const getLocation = async () => {
    try {
      ...
      const response = await fetch('http://172.20.10.3:5000/weather', {
        method: 'POST',
  ...
  const satelliteUrl = weatherData?.satellite_image?.image_url
    ? weatherData.satellite_image.image_url.startsWith('http')
      ...
      : `http://172.20.10.3:5000${weatherData.satellite_image.image_url.startsWith('/') ? '' : '/'}${weatherData.satellite_image.image_url}`
    : null;
